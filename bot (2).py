...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/nextreminder — see your next scheduled reminder and when it'll fire\n"
        "/testreminder — preview what a reminder message looks like, right now\n"
...
async def nextreminder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Shows the next actually-scheduled reminder for this chat — the exact
    time and message content — so you can verify scheduling worked without
    waiting for it to fire."""
    chat_id = update.effective_chat.id
    register_chat(chat_id)
    if not await require_approved(update):
        return
    prefix_class = f"remind:{chat_id}:"
    prefix_quiz = f"quizremind:{chat_id}:"

    all_jobs = context.application.job_queue.jobs()
    mine = [
        j for j in all_jobs
        if j.name and (j.name.startswith(prefix_class) or j.name.startswith(prefix_quiz)) and j.next_t
    ]

    if not mine:
        await update.message.reply_text(
            "No reminders currently scheduled for you. Make sure you've run "
            "/setup and picked at least one subject — reminders are queued "
            "automatically right after that."
        )
        return

    mine.sort(key=lambda j: j.next_t)
    upcoming = mine[:5]
    lines = [f"You have {len(mine)} reminder(s) scheduled. Next {len(upcoming)}:\n"]
    for j in upcoming:
        when_str = j.next_t.astimezone(IST).strftime("%d %b, %I:%M %p")
        kind = "🧪 Quiz" if j.name.startswith(prefix_quiz) else "📘 Class"
        preview = j.data["text"].split("\n")[0]
        lines.append(f"{kind} — {when_str}\n  {preview}\n")

    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")


async def testreminder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Sends a sample class reminder and quiz reminder RIGHT NOW, just to
    preview the wording/format — doesn't affect real scheduling."""
    chat_id = update.effective_chat.id
    register_chat(chat_id)
    if not await require_approved(update):
        return
    class_quip = random.choice(CLASS_REMINDER_QUIPS).format(mins=REMINDER_MINUTES_BEFORE)
    class_sample = f"{class_quip}\n🕒 2:30 - 4:00\n• SAPM [Batch I] (M chopra) @ 6th floor"
    quiz_quip = random.choice(QUIZ_REMINDER_QUIPS).format(subject="SAPM", label="Quiz-1")
    quiz_sample = f"{quiz_quip}\n🗓 21 Aug, 2:15 p.m @ Orion Classroom"

    await update.message.reply_text(
        "Here's what a real reminder looks like (this is just a preview, "
        "not an actual scheduled one):",
    )
    await update.message.reply_text(class_sample, parse_mode="Markdown")
    await update.message.reply_text(quiz_sample, parse_mode="Markdown")
...
def main():
    ...
    app.add_handler(CommandHandler("nextreminder", nextreminder))
